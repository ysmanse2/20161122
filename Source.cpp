...
int main()
{
	...
	for(int t = 0; t < 3000000; t ++)
	{
		...
		if (world.isInside(i, j) == true) // move robot if available
		{
			...
			//world.getCell(i_old, j_old).q_[action] = world.getCell(i, j).getMaxQ();

			// reset if agent is in final cells
			if (world.getCell(i, j).reward_ != 0.0)
			{
				...
				world.getCell(i_old, j_old).q_[action] = gamma * world.getCell(i, j).getMaxQ();
			}
		}
		else
		{
